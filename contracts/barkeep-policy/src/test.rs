...
/*
 * The policy's real behaviour is exercised end to end against a live account in
 * barkeep-smart-account's tests and on Testnet by
 * scripts/tab-lifecycle-testnet.sh. What is worth pinning here is the shape of
 * the contract itself: that install refuses the rule types the library refuses,
 * so the deployed wrapper does not quietly widen them.
 */
