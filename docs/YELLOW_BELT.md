...
## The PromptRail README, as submitted

Moved here from the repository README on 2026-09-14 so that the README opens
on Barkeep. The text below is the README's Yellow Belt material as it stood at
commit `d6be46f`, unchanged except that relative links now resolve from
`docs/` (`src/` is `packages/web/src/` since the workspace split). Nothing in
it has been re-verified since the dates it states.
...
* [`src/contract/paymentTracker.ts`](../packages/web/src/contract/paymentTracker.ts) is a typed
  ...
  confirmation.
* [`src/components/PaymentTracker.tsx`](../packages/web/src/components/PaymentTracker.tsx) is
...
exactly. See [License](../README.md#license).

![Wallet selection modal with multiple wallets](screenshots/wallet-options.png)
...
Implementation: [src/services/wallet.ts](../packages/web/src/services/wallet.ts)

---

## Error Handling

Every failure funnels through one taxonomy in [src/errors.ts](../packages/core/src/errors.ts),
...
Implementation: [src/components/PaymentTracker.tsx](../packages/web/src/components/PaymentTracker.tsx)
and `fetchContractEvents` in
[src/contract/paymentTracker.ts](../packages/web/src/contract/paymentTracker.ts).
...
![XLM Balance](screenshots/balance-testnet.png)
...
![Successful Testnet Transaction](screenshots/payment-success.png)
...
![Transaction Error](screenshots/payment-error.png)
...
configuration
([src/contract/paymentTracker.ts](../packages/web/src/contract/paymentTracker.ts)). The
network and anchor can be overridden with the `VITE_*` variables in
[.env.example](../packages/web/.env.example); see [TRY on/off-ramp via SEP](SEP_RAMP.md).
