...
| 13 | **The auth digest is not account-scoped.** Under `stellar-accounts` 0.7.2 signers sign `sha256(signature_payload ‖ context_rule_ids.to_xdr())`, and `signature_payload` carries no authorizer address. An agent signature is therefore replayable against a different account that lists the same session key, given the same nonce, expiration ledger and invocation. Upstream [#876](https://github.com/OpenZeppelin/stellar-contracts/issues/876), fixed by [#868](https://github.com/OpenZeppelin/stellar-contracts/pull/868) on the unpublished v0.9.0 branch | A property gap in the deployed account, not a theoretical one. Practical exposure today is nil: Testnet, one smart account, every key ours. Separately, every invocation we sign names the account (it is the contract for `add_context_rule`/`remove_context_rule`, and `from` for `transfer`), so the payload already differs per account. That is a side effect of our call shapes, not something the contract guarantees. Demonstrated on Testnet with two throwaway 0.7.2 accounts: a signature for one was [accepted by the other](https://stellar.expert/explorer/testnet/tx/512dbcbfa209f5fe92d36542500417c1ec9d23d9ad87cbfb375941c7a25b2be4), and the same replay on 4529d70 was [refused](https://stellar.expert/explorer/testnet/tx/dbeb6f62c96e00fb684be29ca6d5ea6e01993ef8ca4e9e15210987be427f0cdf) (`deployments/testnet-v09.json`) | Testnet only. Migrate to `AuthDigestPreimage` when 0.9.0 publishes; [MIGRATION-stellar-accounts-0.9.0.md](MIGRATION-stellar-accounts-0.9.0.md) lists what breaks |
