...
3003. I read the return type and the branch and believed the story they told.
A bad signature comes back as `false` and surfaces as 3003. So a client could
tell "the account rejected this signature" apart from a trap somewhere else. I
did not look at what the host function underneath actually does.

## What I observed

`verify` returns `true` or does not return. `verifiers::ed25519::verify` calls
the host's `ed25519_verify`. The host function traps with
`Error(Crypto, InvalidInput)` on a bad signature instead of returning `false`.
Its documentation says so. So the `!verify(..)` branch in `authenticate` is
unreachable with this verifier, and 3003 never appears.
...
For a client this means a wrong digest, a tampered signature and a signature
replayed from another account all read identically on chain:
`Error(Crypto, InvalidInput)` followed by `Error(Auth, InvalidAction)`. On the
`v0.9.0` branch I ran three such cases. I signed the raw `signature_payload`
instead of the digest. I signed the 0.7.2 digest instead of the preimage
digest. I replayed one account's signature against another. Their diagnostic
...
That script simulates the known-good fixture and the flipped-byte fixture. It
...
I submitted each refused case with a footprint borrowed from a correctly
signed copy, so the refusal is the ledger's and not a simulation's. The second,
enforcing simulation caught the same three refusals before submission too.

What I did not run: a signature made with a different key over the correct
...
failure path. But that is inferred, not measured. I also ran the on-chain
cases through the `4529d70` verifier `CD3353…NNS2`, which wraps the same
library function as the 0.7.2 one. The header comment and the tests in the two
crates are identical.
...
`__check_auth`, so it cannot catch any of this. The second simulation caught
every case above.
