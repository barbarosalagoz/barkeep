...
I saw the reports and stopped there. So my architecture plan said the funds
"sit in an audited OpenZeppelin smart account". The intro said the cap was
"enforced on-chain by an OpenZeppelin smart account". Three crate manifests
called `stellar-accounts` "the audited crate". I had transferred someone
else's audit to my deployment without noticing I had done it.

## What I observed

Two separate errors, and they compounded.
...
to audited code. So "pin the audited version" was not an option I had. What
changed between `239a2a7` and 0.7.2 is not recorded in this repo. What I did
record, when I built the `4529d70` crates, is that `spending_limit` changed
again after 0.7.2 (zero-amount transfers now always pass). That is the kind of
change a four-tag gap can hold.

So the audits are evidence about the cryptographic and storage building blocks
my contracts call into, at a nearby commit. They say nothing about my verifier
contracts, my account contract, my policies, how I wire them together, or the
four-tag delta in the library itself.

I reversed the claim in commit 243961c. I added a new §4.1 to
`docs/ARCHITECTURE-v2.md` with a table of every deployed contract and its
audit status, all "No". I put `"audited": false` on every contract in
`deployments/testnet.json`. I made `docs/DEPLOYMENTS.md` lead with the
position. I corrected the manifests and the policy crate's header. Since then
every new contract has carried the same line at the top of its `lib.rs`.

## How I measured it

Not a transaction. The record is documentary.

- The absence of `#[contract]` outside tests in `stellar-accounts` 0.7.2 is
  stated in commit db87031 and `docs/ARCHITECTURE-v2.md` §4. The repo does not
  record the grep. The consequence is checkable, though: `contracts/barkeep-*`
  exist because there was nothing else to deploy.
- The audit reports and the commit each covers are stated in
  `docs/ARCHITECTURE-v2.md` §4.1 as seven reports and `239a2a7` for the
  latest. The repo does not record the URL or the command I used to read the
...
- What is deployed is in `deployments/testnet.json`, `contracts`, one entry per
...
it is first-party. Say what of yours it does not cover.

A related trap for anyone pinning by git: the `v0.9.0` branch at `4529d70`
still reports version 0.7.1 in its crate metadata. A version string on its own
does not tell you what code you have.
