/*
 * Done-test for the tab on stellar-accounts 4529d70 (#868, AuthDigestPreimage).
 *
 * tab-lifecycle-testnet.mjs, unchanged in what it proves, pointed at the 4529d70
 * deployment in deployments/testnet-v09.json and signing with authDigestV09
 * instead of the 0.7.2 authDigest. The only functional difference is the
 * signed digest, which now binds the account address. The 0.7.2 script and its
 * recorded hashes in testnet.json are left as they are.
 *
 * Installs an agent session-key rule with a spending limit and an expiry on the
 * v09 smart account, then proves three things ON CHAIN:
 *
 ...
 *   npx tsx packages/mcp-server/scripts/v09-tab-lifecycle-testnet.mjs
 *
 * Run from packages/mcp-server, for the same @stellar/stellar-sdk 16.3 reason
 * as the 0.7.2 script.
 *
...
import { buildAuthPayload } from "../src/authDigest.ts";
import { authDigestV09 } from "../src/authDigestPreimage.ts";
...
/**
 * Sign one auth entry as the smart account, for a given context rule. The
 * digest binds the account, so it is computed for C.smartAccount.id.
 */
const signAs = (kp, ruleId) => async (_preimage, payload) => ({
  signatureScVal: buildAuthPayload(
    [{ signer: signerOf(kp), signature: kp.sign(authDigestV09(C.smartAccount.id, payload, [ruleId])) }],
    [ruleId]
  ),
});
